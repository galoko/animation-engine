...
CubicSpline<TerrainShaper::Point *> *TerrainShaper::buildMountainRidgeSplineWithPoints(
...
float TerrainShaper::offset(TerrainShaper::Point *value) {
    return this->_offsetSampler->apply(value) + -0.50375F;
}

float TerrainShaper::factor(TerrainShaper::Point *value) {
    return this->_factorSampler->apply(value);
}

float TerrainShaper::jaggedness(TerrainShaper::Point *value) {
    return this->_jaggednessSampler->apply(value);
}

TerrainShaper::Point *TerrainShaper::makePoint(float continents, float erosion, float weirdness) {
