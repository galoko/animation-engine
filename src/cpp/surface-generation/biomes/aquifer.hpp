...
private:
    ...
    static constexpr int32_t SURFACE_SAMPLING_OFFSETS_IN_CHUNKS[][2] = {{-2, -1}, {-1, -1}, {0, -1}, {1, -1}, {-3, 0},
                                                                        {-2, 0},  {-1, 0},  {0, 0},  {1, 0},  {-2, 1},
                                                                        {-1, 1},  {0, 1},   {1, 1}};
...
public:
    Aquifer::FluidStatus *computeFluid(int32_t x, int32_t y, int32_t z) {
        ...
        for (const int32_t(&offset)[2] : SURFACE_SAMPLING_OFFSETS_IN_CHUNKS) {
            ...
            if (isMaxSurfaceYinYRange || isOffsetInPlace) {
                Aquifer::FluidStatus *fluidStatus =
                    this->globalFluidPicker->computeFluid(shiftedX, maxSurfaceY, shiftedZ);
        ...
        if (floodedness > minFloodedness) {
            ...
            double d5 = Mth::map(t, 1.0, 0.0, -0.8, 0.4);
            if (floodedness <= d5) {
                ...
                double fluidLevelSpread =
                    this->fluidLevelSpreadNoise->getValue((double)scaledX, (double)scaledY / 1.4, (double)scaledZ) *
                    10.0;
